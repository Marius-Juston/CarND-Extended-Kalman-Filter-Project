...
void SVD<MatrixType>::compute(const MatrixType &matrix) {
  ...
  for (k = 0; k < (std::max)(nct, nrt); ++k) {
    ...
    for (j = k + 1; j < n; ++j) {
      if ((k < nct) && (m_sigma[k] != 0.0)) {
        ...
        Scalar t =
            matA.col(k).end(m - k).eigen2_dot(matA.col(j).end(m - k)); // FIXME dot product or cwise prod + .sum() ??
        t = -t / matA(k, k);
  ...
  if (wantu) {
    ...
    for (k = nct - 1; k >= 0; k--) {
      if (m_sigma[k] != 0.0) {
        for (j = k + 1; j < nu; ++j) {
          Scalar t = m_matU.col(k).end(m - k).eigen2_dot(m_matU.col(j).end(
              m - k)); // FIXME is it really a dot product we want ?
          t = -t / m_matU(k, k);
