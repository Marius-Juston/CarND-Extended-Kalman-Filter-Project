...
static Index init_rows_cols  /* returns true if OK, or false otherwise */
  ...
  if (stats[COLAMD_STATUS]
      == COLAMD_OK_BUT_JUMBLED) { COLAMD_DEBUG0 (("colamd: reconstructing column form, matrix jumbled\n"));
...
static void init_scoring
  ...
  dense_col_count = std::max<Index>(0,
                                    (std::min)(Index(knobs[COLAMD_DENSE_COL] * n_row),
                                               n_row));COLAMD_DEBUG1 (("colamd: densecount: %d %d\n", dense_row_count, dense_col_count));
  max_deg = 0;
  n_col2 = n_col;
  n_row2 = n_row;
