...
//  const Index PeelAlignedMask = ResPacketSize*peels-1;
  ...
  const Index
      alignmentStep = LhsPacketSize > 1 ? (LhsPacketSize - lhsStride % LhsPacketSize) & LhsPacketAlignedMask : 0;
...
//   const Index PeelAlignedMask = RhsPacketSize*peels-1;
  ...
  for (Index i = skipRows; i < rowBound; i += rowsAtOnce) {
    ...
    if (Vectorizable) {
      ...
      if (alignedSize > alignedStart) {
        switch (alignmentPattern) {
          case AllAligned: for (Index j = alignedStart; j < alignedSize;
                                j += RhsPacketSize) _EIGEN_ACCUMULATE_PACKETS(d, d, d);
            break;
          case EvenAligned: for (Index j = alignedStart; j < alignedSize; j += RhsPacketSize) _EIGEN_ACCUMULATE_PACKETS(d,
                                                                                                                        du,
                                                                                                                        d);
            break;
          case FirstAligned: {
          ...
          default: for (Index j = alignedStart; j < alignedSize; j += RhsPacketSize) _EIGEN_ACCUMULATE_PACKETS(du,
                                                                                                               du,
                                                                                                               du);
            break;
        }
