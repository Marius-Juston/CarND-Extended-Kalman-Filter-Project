...
class CoeffBasedProduct
 ...
 public:
  ...
  const Diagonal<const LazyCoeffBasedProductType,
                 0> diagonal() const { return reinterpret_cast<const LazyCoeffBasedProductType &>(*this); }

  template<int DiagonalIndex>
  const Diagonal<const LazyCoeffBasedProductType,
                 DiagonalIndex> diagonal() const { return reinterpret_cast<const LazyCoeffBasedProductType &>(*this); }
...
struct product_coeff_vectorized_unroller {
  ...
  static EIGEN_STRONG_INLINE void run(Index row,
    ...
    pres = padd(pres,
                pmul(lhs.template packet<Aligned>(row, UnrollingIndex),
...
struct product_packet_impl<RowMajor, UnrollingIndex, Lhs, Rhs, Packet, LoadMode> {
  ...
  static EIGEN_STRONG_INLINE void run(Index row, Index col, const Lhs &lhs, const Rhs &rhs, Packet &res) {
    ...
    res = pmadd(pset1<Packet>(lhs.coeff(row, UnrollingIndex - 1)),
                rhs.template packet<LoadMode>(UnrollingIndex - 1, col),
                res);
  }
};
...
struct product_packet_impl<ColMajor, UnrollingIndex, Lhs, Rhs, Packet, LoadMode> {
  ...
  static EIGEN_STRONG_INLINE void run(Index row, Index col, const Lhs &lhs, const Rhs &rhs, Packet &res) {
    ...
    res = pmadd(lhs.template packet<LoadMode>(row, UnrollingIndex - 1),
                pset1<Packet>(rhs.coeff(UnrollingIndex - 1, col)),
                res);
  }
};
