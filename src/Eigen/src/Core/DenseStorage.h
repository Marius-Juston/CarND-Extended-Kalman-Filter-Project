...
class DenseStorage<T, Dynamic, _Rows, Dynamic, _Options> {
 ...
 public:
  ...
  DenseStorage(DenseIndex size, DenseIndex, DenseIndex nbCols) : m_data(internal::conditional_aligned_new_auto<T,
                                                                                                               (_Options
                                                                                                                   & DontAlign)
                                                                                                                   == 0>(
      size)), m_cols(nbCols) { EIGEN_INTERNAL_DENSE_STORAGE_CTOR_PLUGIN }
#ifdef EIGEN_HAVE_RVALUE_REFERENCES
  DenseStorage(DenseStorage &&other)
...
class DenseStorage<T, Dynamic, Dynamic, _Cols, _Options> {
 ...
 public:
  ...
  DenseStorage(DenseIndex size, DenseIndex nbRows, DenseIndex) : m_data(internal::conditional_aligned_new_auto<T,
                                                                                                               (_Options
                                                                                                                   & DontAlign)
                                                                                                                   == 0>(
      size)), m_rows(nbRows) { EIGEN_INTERNAL_DENSE_STORAGE_CTOR_PLUGIN }
#ifdef EIGEN_HAVE_RVALUE_REFERENCES
  DenseStorage(DenseStorage &&other)
