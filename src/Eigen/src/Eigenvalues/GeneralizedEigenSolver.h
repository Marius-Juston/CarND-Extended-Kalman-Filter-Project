  ...
  if (m_realQZ.info() == Success) {
    ...
    while (i < A.cols()) {
      if (i == A.cols() - 1 || m_matS.coeff(i + 1, i) == Scalar(0)) {
        ...
        // T =  [a b ; 0 c]
        // S =  [e f ; g h]
        RealScalar a = m_realQZ.matrixT().coeff(i, i), b = m_realQZ.matrixT().coeff(i, i + 1),
            c = m_realQZ.matrixT().coeff(i + 1, i + 1);
        RealScalar e = m_matS.coeff(i, i), f = m_matS.coeff(i, i + 1), g = m_matS.coeff(i + 1, i),
            h = m_matS.coeff(i + 1, i + 1);
        RealScalar d = c - a;
        RealScalar gb = g * b;
